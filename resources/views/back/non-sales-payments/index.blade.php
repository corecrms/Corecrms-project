...
@section('title', 'Non Sale Invoice Payments')
@section('style')
    ...
    <div class="content">
        <div class="container-fluid pt-4 px-4 mb-5">
            <div class="border-bottom">
                <h3 class="all-adjustment text-center pb-2 mb-0">All Non Invoice Payments</h3>
            </div>
            @if (session('success'))
            ...
            <div class="card border-0 card-shadow rounded-3 p-2 mt-4">
                <div class="card-header border-0 bg-white">
                    ...
                    <div class="row my-3">
                        ...
                        <div class="col-md-8 col-12 text-end">
                            ...
                            <a href="{{ route('non-sales-payments.create') }}" class="btn create-btn rounded-3 mt-2">Create
                ...
                <div class="table-responsive">
                    ...
                    <table class="table " id="example">
                        ...
                        <tbody>
                            @forelse ($NonSalesInvoicePayments as $salesInvoice)
                                <tr>
                                    ...
                                    <td class="align-middle">
                                        {{ $salesInvoice->customer->user->name ?? ''}}
                                    </td>
                                    <td class="align-middle">{{ $salesInvoice->payment_type ?? '' }}</td>
                                    <td class="align-middle">${{ $salesInvoice->amount_pay }}</td>
                                    {{-- <td class="align-middle">{{ $salesInvoice?->account?->name }}</td> --}}
                                    <td class="align-middle">{{ $salesInvoice?->payment_method ?? '' }}</td>
                                    <td class="align-middle">{{ $salesInvoice->payment_date }}</td>
                                    <td class="align-middle">{{ $salesInvoice->status === 1 ? 'Yes' : 'No' }}</td>
                                    <td class="align-middle">
                                        <div class="">
                                            ...
                                            <div class="dropdown-menu " aria-labelledby="dropdownMenuLink">
                                                {{-- <a class="dropdown-item"
                                                    href="{{ route('non-sales-payments.edit', $salesInvoice->id) }}">
                                                ...
                                                <form class=""
                                                    action="{{ route('non-sales-payments.destroy', $salesInvoice->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
            ...
            $('#deleteRowTrigger').on("click", function(event) { // triggering delete one by one
                if (confirm("Are you sure you won't be able to revert this!")) {
                    if ($('.deleteRow:checked').length > 0) { // at-least one checkbox checked
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{ route('non-sales.multiple.delete') }}",
                            data: {
                                ids
                            },
