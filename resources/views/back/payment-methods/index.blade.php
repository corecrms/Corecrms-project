    ...
    <div class="content">
        ...
        <div class="container-fluid pt-4 px-4 mb-5">
            <div class="border-bottom">
                <h3 class="all-adjustment text-center pb-2 mb-0">Payment Methods</h3>
            </div>
                    ...
                    <table id="example" class="table mb-0">
                        <thead>
                            <tr>
                                ...
                                <th class="sorting">Short Code </th>
                        ...
                        <tbody>
                            ...
                            @forelse ($payments as $payment)
                                <tr>
                                    <td class="align-middle">
                                        <label for="select-checkbox" class="checkbox">
                                            <input class="checkbox__input select-checkbox deleteRow" type="checkbox"
                                                id="select-checkbox" data-id="{{$payment->id}}"/>
                                    ...
                                    <td class="align-middle">{{ $payment->short_code ?? '' }}</td>
                                    <td class="align-middle">{{ $payment->note ?? '' }}</td>

                                    <td class="align-middle">{{ $payment->status == 1 ? 'Active':"Inactive" }}</td>
                                    <td class="align-middle">

                                        <div class="d-flex justify-content-start">

                                            <a class=" text-decoration-none btn edit-category-btn" data-bs-toggle="modal"
                                                data-bs-target="#editCategoryModel{{ $payment->id }}">
                                            ...
                                            <form class="d-inline delete-category-form" method="post"
                                                action="{{ route('categories.destroy', $payment->id) }}">
                                                @csrf
                                                @method('DELETE')
                    ...
                    <div class="modal-body">
                        <form action="{{ route('payment-methods.store') }}" method="POST">
                            ...
                            <div class="form-group mt-2">
                                <label for="exampleFormControlInput1">Short Code</label>
                                <input type="text" class="form-control subheading" name="short_code"
                            ...
                            <div class="form-group mt-2">
                                ...
                                <textarea class="form-control subheading" id="exampleFormControlTextarea1" name="note" rows="3"
                                    required></textarea>
                            </div>
        ...
        @foreach ($payments as $payment)
            <div class="modal fade" id="editCategoryModel{{ $payment->id }}" aria-hidden="true"
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form action="{{ route('payment-methods.update', $payment->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <input type="hidden" id="cate_id_input">
                                <div class="form-group">
                                    <label for="cate_name_input"> Name</label>
                                    <input type="text" class="form-control subheading" name="name"
                                        id="cate_name_input" placeholder="Name" required value="{{ $payment->name ?? '' }}">
                                </div>

                                <div class="form-group mt-2">
                                    <label for="cate_code_input">Short Code</label>
                                    <input type="text" class="form-control subheading" name="short_code"
                                        id="cate_code_input" placeholder="" required value="{{ $payment->short_code ?? '' }}">
                                </div>
                                <div class="form-group mt-2">
                                    <label for="exampleFormControlInput1">Status</label>
                                    <select name="status" id="status" class="form-control">
                                ...
                                <div class="form-group mt-2">
                                    ...
                                    <textarea class="form-control subheading" id="cate_desc_input" name="note" rows="3" >{{ $payment->note }}</textarea>
                                </div>
            ...
            $('#deleteRowTrigger').on("click", function(event){ // triggering delete one by one
                if(confirm("Are you sure you won't be able to revert this!")){
                    if( $('.deleteRow:checked').length > 0 ){  // at-least one checkbox checked
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{route('payment_method.delete')}}",
                            data: {ids},
