    ...
    <div class="container border-one">
        ...
        <div class="info">
            <table>
                <tr>
                    ...
                    <td class="right">
                        <h4>
                            {{ $purchase->warehouse->users->name ?? '' }}
                        </h4>
                        <p>{{ $purchase->warehouse->users->address ?? 'Address' }}<br>Phone:
                            {{ $purchase->warehouse->users->contact_no ?? '1234567' }}<br>{{ $purchase->warehouse->users->email ?? '' }}
                        </p>
                    </td>
        ...
        <div class="info">
            <table>
                <tr>
                    <td class="left">
                        Customer: {{ $purchase->vendor->user->name ?? 'Name' }}<br>
                        Address: {{ $purchase->vendor->user->address ?? 'Address' }}<br>
                        Phone: {{ $purchase->vendor->user->contact_no ?? '454543' }}<br>
                        Email {{ $purchase->vendor->user->email ?? 'example@gmail.com' }}
                    </td>
            ...
            <table>
                ...
                <tbody>
                    ...
                    @foreach ($paymentInvoice->invoicePayment as $invoice)
                        <tr>
                            <td>{{ $invoice->purchaseInvoice->invoice_id ?? '102883' }}</td>
                            <td>{{ $invoice->created_at ? $invoice->created_at->format('F j, Y') : '2021-09-09' }}</td>
                            <td>{{$paymentInvoice->payment_method ?? 'Method'}}</td>
                            <td>${{ $invoice->purchaseInvoice->purchase->grand_total ?? '0' }}</td>
                            <td>${{ $invoice->paid_amount ?? '0' }}</td>
                            <td>${{ $invoice->purchaseInvoice->purchase->amount_due ?? '0' }}</td>
                        </tr>
