...
@section('content')
    ...
    <div class="content">
        ...
        <div class="container-fluid pt-4 px-4 mb-5">
            <div class="border-bottom">
                <h3 class="all-adjustment text-center pb-2 mb-0">Sale Payment Report</h3>
            </div>
                        ...
                        <tbody>
                            ...
                            @foreach ($sales as $sale)
                                <tr>
                                    ...
                                    <td>{{ $sale['date'] }}</td>
                                    <td><a href="/sales/{{ $sale['sale_id'] }}">{{ $sale['invoice_id'] }}</a></td>
                            ...
                            {{-- @foreach ($salesInvoicePayments as $sale)
                                <tr>
                                    <td>
                                        <label for="myCheckbox09" class="checkbox">
                                            <input class="checkbox__input" type="checkbox" id="myCheckbox09" />
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="staticBackdrop" aria-labelledby="staticBackdropLabel"
        ...
        <div class="offcanvas-body">
            <form action="{{ route('sale.payment.filter.customer') }}" method="POST">
                @csrf
                @method('POST')
                <div>
                    <div class="form-group">
                        ...
                        <select class="form-control form-select subheading mt-1" aria-label="Default select example"
                            ...
                            @foreach ($customers as $customer)
                                <option value="{{ $customer->id }}" {{request()->customer_id == $customer->id ? 'selected':''}} >{{ $customer->user->name }}</option>
                            @endforeach
                        </select>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('input[name="dates"]').on('apply.daterangepicker', function(ev, picker) {
                ...
                let baseUrl = "{{ route('sale.payment.filter') }}";
