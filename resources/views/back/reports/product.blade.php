            ...
            <div class="card border-0 card-shadow rounded-3 p-2 mt-5">
                    ...
                    <div class="row my-3">
                        <div class="col-md-3 col-12 mt-2">
                            <div class="input-search position-relative">
                                <input type="text" placeholder="Search Product" class="form-control rounded-3 subheading"
                                    id="custom-filter" />
                ...
                <div class="table-responsive">
                    <table class="table" id="example">
                        ...
                        <tbody>
                            @foreach ($product_reports as $product_item)
                                <tr>
                                    <td class="align-middle">{{ $product_item->sku }}</td>
                                    <td class="text-primary align-middle">{{ $product_item->product }}</td>
                                    <td class="align-middle">{{ $product_item->total_sales }} {{ $product_item->unit_name ?? '' }}</td> <!-- This is now in the parent unit -->
                                    <td class="align-middle">${{ $product_item->total_amount }}</td>
                                    <td class="align-middle">
                                        <a class="btn btn-sm rounded-3 save-btn btn-primary" href="{{route('product_item.show', $product_item->id)}}" class="btn btn-primary btn-sm">Report</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('input[name="dates"]').on('apply.daterangepicker', function(ev, picker) {
                ...
                let baseUrl = "{{ route('product.filter') }}";
            ...
            var table = $('#example').DataTable({
                ...
                processing: true,
                serverSide: false,
                data: {!! json_encode($product_reports) !!},
                columns: [

                   {
                    ...
                    // }
                    {
                        ...
                        render: function(data, type, row) {
                            ...
                            return `<a href="/product_item/show/${data}" class="btn btn-sm rounded-3 save-btn btn-primary">Report</a>`;
                        }
                    }
                ],
                // buttons: [{
