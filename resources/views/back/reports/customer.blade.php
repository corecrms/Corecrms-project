                    ...
                    <div class="row my-3">
                        <div class="col-md-3 col-12 mt-2">
                            <div class="input-search position-relative">
                                <input type="text" placeholder="Search Customer" class="form-control rounded-3 subheading"
                                    id="custom-filter" />
                        ...
                        <div class="col-md-9 col-12 text-end">
                            ...
                            <a href="{{ route('customers.create') }}" class="btn create-btn rounded-3 mt-2">Create <i
                    ...
                    <table class="table" id="example">
                        <thead>
                            <tr>
                                ...
                                <th class="text-secondary">Total Due</th>
                                <th class="text-secondary">Credit Amount</th>
                        ...
                        <tbody>
                            @foreach ($customers as $customer)
                                <tr>
                                    <td class="align-middle">{{ $customer['customer'] }}</td>
                                    <td class="align-middle">{{ $customer['total_sales'] }}</td>
                                    <td class="align-middle">${{ $customer['total_amount'] }}</td>
                                    <td class="align-middle">${{ $customer['total_paid'] }}</td>
                                    <td class="align-middle">${{ $customer['total_due'] }}</td>
                                    <td class="align-middle">${{ $customer['credit_amount'] }}</td>
                                    <td class="align-middle d-flex align-items-center">
                                        <a href="{{route('customer.download-pdf',$customer['id'])}}">
                                            <i class="bi bi-file-earmark-pdf fs-5 pe-2"></i>
                                        </a>
...
<script type="text/javascript" src="https://cdn.jsdelivr.net/npm/daterangepicker/daterangepicker.min.js"></script>
    ...
    <script>
        $(document).ready(function() {
            ...
            //on click .applyBtn ajax call for filter data of customers between date range
            $('input[name="dates"]').on('apply.daterangepicker', function(ev, picker) {
                var start = picker.startDate.format('YYYY-MM-DD');
                var end = picker.endDate.format('YYYY-MM-DD');
                let baseUrl = "{{ route('customer.filter') }}";
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            ...
                            return `<a href="/customer/${row.id}/download-pdf">
                                            <i class="bi bi-file-earmark-pdf fs-5 pe-2"></i>
                                        </a>
                                        <a href="/customer/report/detail_customer/${row.id}">
                                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="M1 12C1 12 5 4 12 4C19 4 23 12 23 12C23 12 19 20 12 20C5 20 1 12 1 12Z"
