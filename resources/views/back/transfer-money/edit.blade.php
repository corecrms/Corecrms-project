                ...
                <div class="card-header bg-white border-0 rounded-3 p-4">
                    <div class="">
                        <div class="create-product-form rounded">
                            <form action="{{ route('transfer-money.update', $transferMoney->id) }}" method="POST" enctype="multipart/form-data">
                                @method('PUT')
                                @csrf
                                <div class="row">
                                    <div class="col-md-4">
                                      <div class="form-group">
                                        ...
                                        <input type="date" class="form-control subheading" id="date" name="date" value="{{ $transferMoney->date }}" required>
                                      </div>
                                    </div>
                                    <div class="col-md-4">
                                      <div class="form-group">
                                        ...
                                        <select class="form-control form-select subheading" name="account_id" id="account_id" required>
                                          ...
                                          @forelse ($accounts as $account)
                                              <option value="{{ $account->id }}" {{ $account->id == $transferMoney->account_id ? 'selected' : '' }}>{{ $account->name }}</option>
                                            @empty
                                    ...
                                    <div class="col-md-4">
                                      <div class="form-group">
                                        ...
                                        <select class="form-control form-select subheading" name="expense_category_id" id="expense_category_id" required> 
                                          ...
                                          @forelse ($transferMoneyCategories as $transferMoneyCategory)
                                              <option value="{{ $transferMoneyCategory->id }}" {{ $transferMoneyCategory->id == $transferMoney->expense_category_id ? 'selected' : '' }}>{{ $transferMoneyCategory->name }}</option> 
                                            @empty
                                ...
                                <div class="row mt-2">
                                    <div class="col-md-4">
                                      <div class="form-group">
                                        ...
                                        <input type="number" placeholder="Amount" class="form-control subheading" id="amount" name="amount" required value="{{ $transferMoney->amount }}">
                                      </div>
                                    </div>
                                </div>
                                <div class="form-group mb-4">
                                    <label for="description">Description</label>
                                    <textarea class="form-control" id="description" name="description" rows="3">{{ $transferMoney->description }}</textarea>
                                </div>
