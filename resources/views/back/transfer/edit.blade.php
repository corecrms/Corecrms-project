    ...
    <div class="content">
        ...
        <div class="container-fluid pt-4 px-4 mb-5">
            <div class="border-bottom">
                <h3 class="all-adjustment text-center pb-2 mb-0">Update Transfer</h3>
            </div>
            <form class="container-fluid" action="{{ route('transfers.update', $transfer->id) }}" method="POST" id="createSaleForm">
                @csrf
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input class="form-control subheading" type="date" value="{{$transfer->date}}" />
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select class="form-control form-select subheading mt-1"
                                        aria-label="Default select example" name="from_warehouse_id"id="from_warehouse_id" disabled>
                                        <option value="">Select Warehouse</option>
                                        @foreach ($warehouses as $warehouse)
                                            <option value="{{ $warehouse->id }}" {{$transfer->from_warehouse->id == $warehouse->id ? 'selected': ''}}>{{ $warehouse->users->name ?? ''}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select class="form-control form-select subheading mt-1"
                                        ...
                                        <option value="">Select Warehouse</option>
                                        @foreach ($warehouses as $warehouse)
                                            <option value="{{ $warehouse->id }}" {{$transfer->to_warehouse->id == $warehouse->id ? 'selected': ''}}>{{ $warehouse->users->name ?? ''}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <table class="table text-center" id="mainTable">
                            ...
                            <tbody>
                                @foreach ($transfer->transfer_products as $product)
                                    <tr>
                                        @php
                                        $warehouse_product = \App\Models\ProductWarehouse::where('product_id',$product->product_id)->where('warehouse_id',$transfer->from_warehouse->id)->first();
                                        // return $quantity;
                                        @endphp
                                        <td class="align-middle">{{$loop->iteration}}</td>
                                        <td class="product_name align-middle " data-product='{{$product->products}}'>{{$product->products->name}}</td>
                                        <td class=" align-middle ">{{$product->products->sku ?? ''}}</td>
                                        <td class="product_sell_price align-middle ">{{$product->products->sell_price ?? ''}}</td>
                                        <td class="align-middle">
                                            <span class="badges bg-darkwarning p-1" product_stock >{{$warehouse_product->quantity ?? ''}} {{$product->products->unit?->short_name ? $product->products->unit?->short_name : ''}}</span>
                                        </td>
                                        <td class="align-middle">
                                        <div
                                        ...
                                        <td class="align-middle">0.00</td>
                                        <td class="product_price align-middle">{{$product->subtotal}}</td>
                                        <td class=" align-middle">
                    ...
                    <div class="row mt-2 px-3">
                        ...
                        <div class="col-md-4 border rounded-2">
                            <div class="row border-bottom subheading">
                                ...
                                <div class="col-md-6 col-6" id="order_tax_display">$0.00</div><span> ({{$transfer->order_tax}}%)</span>
                            </div>

                            <div class="row border-bottom">
                                <div class="col-md-6 col-6">Discount</div>
                                <div class="col-md-6 col-6" id="discount_display">$ {{$transfer->discount ?? ''}}</div>
                            </div>

                            <div class="row border-bottom">
                                <div class="col-md-6 col-6">Shipping</div>
                                <div class="col-md-6 col-6" id="shipping_display">$ {{$transfer->shipping ?? ''}}</div>
                            </div>

                            <div class="row disabled-bg">
                                <div class="col-md-6 col-6">Grand Total</div>
                                <div class="col-md-6 col-6" id="grand_total">{{$transfer->grand_total ?? ''}}</div>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="number" placeholder="0%" class="form-control subheading"value="{{$transfer->order_tax ?? ''}}"
                                        id="order_tax" />
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="number" placeholder="$0.00" class="form-control subheading"value="{{$transfer->discount ?? ''}}"
                                        id="discount" />
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="number" placeholder="$0.00" class="form-control subheading"
                                        id="shipping" value="{{$transfer->shipping ?? ''}}" />
                                </div>
                            </div>
                        </div>
                        <div class="row">
                ...
                <button class="btn save-btn text-white mt-3" type="submit">
                    <span id="btn-text">Update Transfer</span>
