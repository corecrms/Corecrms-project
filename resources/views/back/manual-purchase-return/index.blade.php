...
@section('title', 'Manual Sale Return')
@section('style')
    ...
    <div class="content">
                ...
                <div class="card-header border-0 bg-white">
                    <div class="row my-3">
                        ...
                        <div class="col-md-9 col-12 text-end">
                            ...
                            <a href="{{ route('manual-purchase-return.create') }}"
                                class="btn create-btn rounded-3 mt-2">Create
                        ...
                        <tbody>
                            @foreach ($returns as $return)
                                <tr>
                                    ...
                                    <td class="text-primary align-middle">{{ $return->reference ?? 'N/A' }}</td>
                                    <td class="align-middle">{{ $return->vendor->user->name ?? 'N/A' }}</td>
                                    <td class="align-middle">{{ $return->warehouse->users->name ?? 'N/A' }}</td>
                                    <td class="align-middle">
                                        <span class="badges green-border text-center">{{ $return->status ?? 'N/A' }}</span>
                                    </td>
                                    <td class="align-middle">${{ $return->grand_total ?? 'N/A' }}</td>
                                    <td class="align-middle">${{ $return->amount_paid ?? 'N/A' }}</td>
                                    <td class="align-middle">${{ $return->amount_due ?? 'N/A' }}</td>
                                    <td class="align-middle"><span
                                            class="badges green-border text-center">{{ $return->payment_status ?? 'N/A' }}</span>
                                    </td>
                                        ...
                                        <div>
                                            ...
                                            <div class="dropdown-menu p-2 ps-0" aria-labelledby="dropdownMenuLink">
                                                <a class="dropdown-item"
                                                    href="{{ route('manual-purchase-return.show', $return->id) }}">
                                                    <img src="{{ asset('back/assets/dasheets/img/menu.svg') }}"
                                                        class="img-fluid me-1" alt="" />
                                                    Detail Return
                                                </a>
                                                <a class="dropdown-item"
                                                    href="{{ route('manual-purchase-return.edit', $return->id) }}">
                                                    <img src="{{ asset('back/assets/dasheets/img/menu.svg') }}"
                                                        class="img-fluid me-1" alt="" />
                                                    Edit Return
                                                </a>
                                                ...
                                                <form id="deleteForm"
                                                    action="{{ route('manual-purchase-return.destroy', $return->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
        ...
        <div class="offcanvas offcanvas-end" tabindex="-1" id="staticBackdrop" aria-labelledby="staticBackdropLabel"
            ...
            <div class="offcanvas-body">
                <form action="{{ route('manual_purchase_return.filter') }}" method="POST">
                    @csrf
                    @method('POST')
                    <div>
                        ...
                        <div class="form-group">
                            <label for="customer_id">Vendor</label>
            ...
            $('#deleteRowTrigger').on("click", function(event) { // triggering delete one by one
                if (confirm("Are you sure you won't be able to revert this!")) {
                    if ($('.deleteRow:checked').length > 0) { // at-least one checkbox checked
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{ route('manual_purchase_return.delete') }}",
                            data: {
                                ids
                            },
