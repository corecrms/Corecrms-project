...
@section('content')
    ...
    <div class="content">
        ...
        <div class="container-fluid pt-4 px-4 mb-5">
            ...
            <form class="container-fluid" action="{{ route('manual-purchase-return.update', $purchase_return->id) }}"
                method="POST" id="createSaleForm">
                @csrf
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select class="form-control form-select subheading mt-1"
                                        ...
                                        <option value="received" {{ $purchase_return == 'received' ? 'selected' : '' }}>
                                            Received</option>
                                        <option value="pending" {{ $purchase_return == 'pending' ? 'selected' : '' }}>Pending
                                        </option>
                        ...
                        <table class="table text-center mt-2" id="mainTable">
                            ...
                            <tbody>
                                @foreach ($purchase_return->return_items as $product)
                                    {{-- @php
                                    $item = \App\Models\ProductItem::where('sale_id',$purchase_return->sales->id)->where('product_id',$product->product_id)->first();
                                ...
                                @endphp --}}
                                            ...
                                            if ($product->product->product_unit != $product->purchase_units?->id) {
                                                if ($product->product->unit->parent_id == 0) {
                                                    $expression =
                                                        ...
                                                        $product->return_units?->operator_value;
                                    ...
                                    <tr>
                                        ...
                                        <td class="align-middle"> <span
                                                class="badges bg-darkwarning p-1">{{ $quantity ?? '' }}
                                                {{ $product->return_units?->short_name ? $product->return_units?->short_name : '' }}</span>
                                        </td>
                                        <td class="align-middle">
                                            <div class="quantity d-flex justify-content-center align-items-center">
                                                ...
                                                <input type="number" id="quantityInput"
                                                    ...
                                                    value="{{ $product->return_quantity ?? '' }}" min="0" />
                                                <button type="button" class=" btn qty-plus-btn" id="plusBtn">
                                                    <i class="fa-solid fa-plus"></i>
                                                </button>
                                            </div>
                                        </td>
                                        <td class="align-middle">{{ $purchase_return->discount ?? '' }} </td>
                                        <td class="align-middle">{{ $purchase_return->order_tax ?? '' }} </td>
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#createSaleForm').on('submit', function(e) {
                ...
                $.ajax({
                    url: '/manual-purchase-return/{{ $purchase_return->id }}',
                    type: 'PUT',
                    headers: {
                    ...
                    error: function(xhr) {
                        ...
                        } else if (xhr.status === 400) { // Custom error from the backend
                            var errorMessage = xhr.responseJSON.message;
                            toastr.error(errorMessage); // Display the custom error message
                        } else {
