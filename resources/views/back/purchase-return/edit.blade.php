    ...
    <div class="content">
        ...
        <div class="container-fluid pt-4 px-4 mb-5">
            <div class="border-bottom">
                <h3 class="all-adjustment text-center pb-2 mb-0">Edit Purchase Return</h3>
            </div>
            <form class="container-fluid" action="{{ route('purchase_return.update' , $purchase_return->id) }}" method="POST" id="createSaleForm">
                @csrf
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input class="form-control subheading" type="date" value="{{ $purchase_return->date ?? ''}}" />
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select class="form-control form-select subheading mt-1"
                                        ...
                                        <option value="received" {{$purchase_return == 'received' ? 'selected': ''}}>Received</option>
                                        <option value="pending" {{$purchase_return == 'pending' ? 'selected': ''}}>Pending</option>
                ...
                <div class="card card-shadow rounded-3 border-0 mt-4 p-3">
                    <div class="table-responsive">
                        ...
                        <table class="table text-center mt-2" id="mainTable">
                            <span class="alert alert-danger p-1 m-1 rounded mb-2 w-100">Any products with a quantity set to 0 won't be
                                refunded</span>
                            ...
                            <thead class="fw-bold ">
                                <tr>
                                    ...
                                    <th class="align-middle">Qty purchase</th>
                            ...
                            <tbody>
                                @foreach ($purchase_return->return_items as $product)
                                @php
                                    $item = \App\Models\PurchaseProductItem::where('purchase_id',$purchase_return->purchase->id)->where('product_id',$product->product_id)->first();
                                    $item->load('purchase_unit');
                                    $purchase = \App\Models\Purchase::find($purchase_return->purchase_id);
                                    $warehouse_product = \App\Models\ProductWarehouse::where('product_id',$product->product_id)->where('warehouse_id',$purchase->warehouse_id)->first();
                                ...
                                @endphp
                                    <tr>
                                        ...
                                        <td class="align-middle"> <span class="badges bg-darkwarning p-1">{{ $item->quantity ?? '' }} {{ $item->purchase_units?->short_name ? $item->purchase_units->short_name : ''}}</span></td>
                                        <td class="align-middle">
                                            <span class="badges bg-darkwarning p-1">
                                                {{ number_format($quantity , 3) ?? '' }} {{ $product->product->unit?->short_name ?? '' }} </td>
                                            </span>
                                        <td class="align-middle">
                                        ...
                                        <td class="align-middle">{{ $purchase_return->purchase->discount ?? '' }} </td>
                                        <td class="align-middle">{{ $purchase_return->purchase->order_tax ?? '' }} </td>
                    ...
                    <div class="row mt-2 px-3">
                        ...
                        <div class="col-md-4 border rounded-2">
                            ...
                            <div class="row disabled-bg">
                                ...
                                <div class="col-md-6 col-6" id="grand_total">$ {{$purchase_return->grand_total}}</div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function calculateTotal() {
            ...
            $('.table tbody tr').each(function() {
                const quantity = parseInt($(this).find('td:nth-child(6) input').val()) ||
                0; // Parse quantity as integer
                const price = parseFloat($(this).find('td:nth-child(3)').text()) || 0; // Parse price as float
                const itemSubtotal = quantity * price;
                subtotal += itemSubtotal;
                $(this).find('td:nth-child(9)').text(itemSubtotal.toFixed(2)); // Update subtotal for each row
            });
        ...
        $(document).ready(function() {
            $('#createSaleForm').on('submit', function(e) {
                ...
                $.ajax({
                    url: '/purchase_return/{{$purchase_return->id}}',
                    type: 'PUT',
                    headers: {
