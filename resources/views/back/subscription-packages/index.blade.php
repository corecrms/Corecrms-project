                        ...
                        <table id="example" class="table table-striped align-middle mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th><strong>Duration (months)</strong></th>
                                        ...
                                        <td class="text-center">
                                            <div class="table_switch">
                                                <div class="form-check form-switch">
                                                    <input class="form-check-input status" data-id="{{ $package->id }}"
                                                        type="checkbox" id="flexSwitchCheck{{ $package->id }}"
                                                        {{ $package->status == 1 ? 'checked' : '' }}>
                                                    <label class="form-check-label"
                                                        for="flexSwitchCheck{{ $package->id }}"></label>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="dropdown ms-auto">
                                                <a class="dropdown-toggle dropdown-toggle-nocaret" href="#"
                                                    role="button" id="dropdownMenuLink{{ $package->id }}"
                                                    data-bs-toggle="dropdown" aria-expanded="false">
                                                    <i class='bx bx-dots-horizontal-rounded font-22 text-option'></i>
                                                </a>
                                                <ul class="dropdown-menu"
                                                    aria-labelledby="dropdownMenuLink{{ $package->id }}">
                                                    <li><a class="dropdown-item"
                                                            href="{{ route('subscription-packages.edit', $package->id) }}">Edit</a>
                                                    </li>
                                                    <li>
                                                        <form class="delete-package-form"
                                                            action="{{ route('subscription-packages.destroy', $package->id) }}"
                                                            method="post">
                                                            @csrf
                                                            @method('DELETE')
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            ...
            $(".delete-package-form").submit(function() {
