            ...
            <div class="card card-shadow border-0 mt-5 rounded-3">
                ...
                <div class="table-responsive p-2">
                            ...
                            @foreach ($sales as $sale)
                                <tr>
                                    ...
                                    <td class="text-primary align-middle">
                                        <a class="text-decoration-none" href="{{ route('sales.show', $sale->id) }}">{{ $sale->reference }}</a>
                                    </td>
                                    <td class="text-primary align-middle">
                                        <a class="text-decoration-none" href="{{ route('sales.show', $sale->id) }}">{{ $sale->tracking_number ?? '...' }}</a>
                                    </td>
                                    <td class="text-primary align-middle">
                                        {{-- <a class="text-decoration-none" href="{{ $sale->shipping_label ?? '...' }}">Print Label</a> --}}
                                        @if ($sale->shipping_label)
                                            <a href="{{ $sale->shipping_label }}" class="btn btn-sm save-btn text-white" target="_blank">Print Label</a>
                                        @else
                                            <a href="javascript:void(0);" class="btn btn-sm btn-secondary disabled" aria-disabled="true" tabindex="-1">No available</a>
                                        @endif
                                    </td>
                                    <td class="align-middle">{{ $sale->warehouse->users->name ?? '' }}</td>
                                    <td class="align-middle">{{ $sale->customer->user->name ?? '' }}</td>
                                    <td class="align-middle">{{ $sale->shipping_method ?? '' }}</td>
                                    <td class="align-middle">${{ number_format($sale->shipping ?? 0, 2) }}</td>
                                    <td class="align-middle">${{ number_format($sale->grand_total ?? 0, 2) }}</td>
                                    <td class="align-middle">
                                        @if($sale->tracking_number == null)
                                            <a href="{{ route('create.shipment', $sale->id) }}" class="btn btn-sm save-btn text-white">Create Shipment</a>
                                        @else
                                            <a href="javascript:void(0);" class="btn btn-sm btn-secondary disabled" aria-disabled="true" tabindex="-1">Shipment Created</a>
                                        @endif
                                    </td>
                    ...
                    <div class="d-flex justify-content-center">
                        <div class="spinner-border spinner-dark text-primary" role="status" id="loader"
                            style="display:none;">
