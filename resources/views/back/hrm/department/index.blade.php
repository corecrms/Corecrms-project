    ...
    <div class="content">
        ...
        <div class="container-fluid pt-4 px-4 mb-5">
            <div class="border-bottom">
                <h3 class="all-adjustment text-center pb-2 mb-0">Department</h3>
            </div>
                    ...
                    <table id="example" class="table mb-0">
                        <thead>
                            <tr>
                                ...
                                <th class="align-middle">Department Head</th>
                                    ...
                                    <td class="align-middle">
                                        <label for="select-checkbox" class="checkbox">
                                            <input class="checkbox__input select-checkbox deleteRow" type="checkbox"
                                                id="select-checkbox" data-id="{{$department->id}}"/>
                                    ...
                                    <td class="align-middle">
                                        ...
                                        <div class="d-flex justify-content-start">
                                            ...
                                            <a class=" text-decoration-none btn edit-category-btn" data-bs-toggle="modal"
                                                data-bs-target="#editCategoryModel{{ $department->id }}">
                        ...
                        <form action="{{ route('departments.store') }}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control subheading" name="name"
                                    id="name" placeholder="Department Name"  required>
                            </div>
        ...
        @foreach ($departments as $department)
            <div class="modal fade" id="editCategoryModel{{ $department->id }}" aria-hidden="true"
                        ...
                        <div class="modal-body">
                            <form action="{{ route('departments.update', $department->id) }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    <label for="name">Name</label>
                                    <input type="text" class="form-control subheading" name="name"
                                        id="name" placeholder="Department Name"  required value="{{$department->name ?? ''}}">
                                </div>
                                <div class="form-group mt-2">
                                ...
                                <div class="form-group mt-2">
                                    <label for="update_department_head">Department Head</label>
                                    <select name="department_head" id="update_department_head" class="form-control">
                                        <option value="">Select Department Head</option>
                                        @foreach ($department->company->employees as $employee)
                                            <option value="{{ $employee->id }}" {{$employee->id == $department->department_head ? 'selected':''}}>{{ $employee->first_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
    ...
    <script>
            ...
            $('#deleteRowTrigger').on("click", function(event){ // triggering delete one by one
                if(confirm("Are you sure you won't be able to revert this!")){
                    if( $('.deleteRow:checked').length > 0 ){  // at-least one checkbox checked
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{route('departments.multiple.delete')}}",
                            data: {ids},
        ...
        $(document).ready(function() {
            ...
            $(document).on('change','#company_id',function(){
                ...
                let department_head = $('#department_head');
                department_head.empty();
                department_head.append('<option value="">Select Department Head</option>');
                if(employees){
                    employees.forEach(employee => {
                        department_head.append(`<option value="${employee.id}">${employee.first_name}</option>`);
                    });
                }
            });
            ...
            $(document).on('change','#update_company_id',function(){
                let employees = $(this).find(':selected').data('employees');
                let department_head = $('#update_department_head');
                department_head.empty();
                department_head.append('<option value="">Select Department Head</option>');
                if(employees){
                    employees.forEach(employee => {
