    ...
    <div class="content">
        ...
        <div class="container-fluid pt-4 px-4 mb-5">
            <div class="border-bottom">
                <h3 class="all-adjustment text-center pb-2 mb-0">Attendances</h3>
            </div>
                    ...
                    <table id="example" class="table mb-0">
                        <thead>
                            <tr>
                                ...
                                <th class="align-middle">Work Duration</th>
                        ...
                        <tbody>
                            @forelse ($attendances as $attendance)
                                @php
                                    $working_duration = strtotime($attendance->time_out) - strtotime($attendance->time_in);
                                    $working_duration = gmdate('H:i', $working_duration);
                                @endphp
                                <tr>
                                    <td class="align-middle">
                                        <label for="select-checkbox" class="checkbox">
                                            <input class="checkbox__input select-checkbox deleteRow" type="checkbox"
                                                id="select-checkbox" data-id="{{$attendance->id}}"/>
                                    ...
                                    <td class="align-middle">
                                        {{$attendance->employee->first_name ?? '' . $attendance->employee->first_name ?? ''}}
                                    </td>
                                    <td  class="align-middle">
                                    ...
                                    <td class="align-middle">
                                        ...
                                        <div class="d-flex justify-content-start">
                                            ...
                                            <a class=" text-decoration-none btn edit-category-btn" data-bs-toggle="modal"
                                                data-bs-target="#editCategoryModel{{ $attendance->id }}">
                        ...
                        <form action="{{ route('attendance.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <select name="company_id" id="company_id" class="form-control">
                                    ...
                                    @foreach ($companies as $company)
                                        ...
                                        data-employees="{{$company->employees}}" >{{ $company->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control subheading" name="date"
                                    id="date" placeholder="Company Phone" required>
                            </div>
                            <div class="form-group">
                                <label for="time_in">Time In</label>
                                <input type="time" class="form-control subheading" name="time_in"
                                    id="time_in" placeholder="Company Phone" required>
                            </div>
                            <div class="form-group">
                                <label for="time_out">Time Out</label>
                                <input type="time" class="form-control subheading" name="time_out"
                                    id="time_out" placeholder="Company Phone" required>
                            </div>
        ...
        @foreach ($attendances as $attendance)
            <div class="modal fade" id="editCategoryModel{{ $attendance->id }}" aria-hidden="true"
                        ...
                        <div class="modal-body">
                            <form action="{{ route('attendance.update', $attendance->id) }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="employee_id" id="update_employee_id" class="form-control update_employee_id">
                                        <option value="">Select Employee</option>
                                        @foreach ($employees as $employee)
                                ...
                                <div class="form-group">
                                    <label for="date">Attendance Date</label>
                                    <input type="date" class="form-control subheading" name="date" id="date" placeholder="Attendance Date" value="{{ $attendance->date }}" required>
                                </div>
                                <div class="form-group">
                                    <label for="time_in">Time In</label>
                                    <input type="time" class="form-control subheading" name="time_in" id="time_in" placeholder="Time In" value="{{ $attendance->time_in }}" required>
                                </div>
                                <div class="form-group">
                                    <label for="time_out">Time Out</label>
                                    <input type="time" class="form-control subheading" name="time_out" id="time_out" placeholder="Time Out" value="{{ $attendance->time_out }}" required>
                                </div>
    ...
    <script>
            ...
            $('#deleteRowTrigger').on("click", function(event){ // triggering delete one by one
                if(confirm("Are you sure you won't be able to revert this!")){
                    if( $('.deleteRow:checked').length > 0 ){  // at-least one checkbox checked
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{route('companies.multiplae.delete')}}",
                            data: {ids},
        ...
        $(document).ready(function() {
            ...
            $(".delete-category-form").submit(function() {
                var decision = confirm("Are you sure, You want to Delete this attendance?");
                if (decision) {
                    return true;
                }
                return false;
            ...
            $(document).on('change','#company_id', function (){
                ...
                let employeeSelect = $(this).closest('form').find('#employee_id');
                employeeSelect.html('');
            ...
            $(document).on('change', '.update_company_id', function() {
                let employees = $(this).find(':selected').data('employees') || [];
                let employeeSelect = $(this).closest('form').find('.update_employee_id');
                employeeSelect.html('');
                employeeSelect.append('<option value="">Select Employee</option>');
            ...
            // $('.update_company_id').trigger('change');
