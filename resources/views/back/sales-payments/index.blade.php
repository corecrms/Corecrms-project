    ...
    <div class="content">
        <div class="container-fluid pt-4 px-4 mb-5">
            <div class="border-bottom">
                <h3 class="all-adjustment text-center pb-2 mb-0">All Invoice Payments</h3>
            </div>
            @if (session('success'))
                ...
                <div class="card-header border-0 bg-white">
                    ...
                    <div class="row my-3">
                        ...
                        <div class="col-md-8 col-12 text-end">
                            ...
                            <a href="{{ route('sales-payments.create') }}" class="btn create-btn rounded-3 mt-2">Create <i
                        ...
                        <tbody>
                            @forelse ($salesInvoicePayments as $salesInvoice)
                                ...
                                <tr>
                                    <td class="align-middle">
                                        <label for="select-checkbox" class="checkbox">
                                            <input class="checkbox__input select-checkbox deleteRow" type="checkbox"
                                                id="select-checkbox" data-id="{{ $salesInvoice->id }}" />
                                    ...
                                    <td class="align-middle">
                                        <a href="{{ route('sales.show', $salesInvoice->saleInvoice->sale->id) }}"
                                            class="text-decoration-none">{{ $salesInvoice->saleInvoice->invoice_id }}</a>
                                    </td>
                                    {{-- <td class="align-middle">
                                        {{ $salesInvoice->saleInvoice->invoice_id }}
                                    </td> --}}
                                    <td class="align-middle">
                                        {{ $salesInvoice->saleInvoice->sale->customer->user->name ?? '' }}</td>
                                    <td class="align-middle">${{ $salesInvoice->saleInvoice->sale->grand_total ?? '' }}
                                    </td>
                                    <td class="align-middle">${{ $salesInvoice->paid_amount }}</td>
                                    {{-- <td class="align-middle">{{ $salesInvoice->salesPayment->account?->name ?? '...' }} --}}
                                    <td class="align-middle">{{ $salesInvoice->salesPayment->payment_method ?? '...' }}</td>
                                    <td class="align-middle">{{ $salesInvoice->salesPayment->payment_date }}</td>
                                    <td class="align-middle">
                                        {{ $salesInvoice->salesPayment->status === 1 ? 'Yes' : 'No' }}</td>
                                    <td class="align-middle">
                                        <div class="">
                                            ...
                                            <div class="dropdown-menu " aria-labelledby="dropdownMenuLink">
                                                {{-- <a class="dropdown-item"
                                                    href="{{ route('get-one-product-details', $salesInvoice->id) }}">
                                                    <img src="{{ asset('back/assets/dasheets/img/menu.svg') }}"
                                                        class="img-fluid me-1" alt="" />
                                                    Product Detail
                                                </a> --}}
                                                <a class="dropdown-item"
                                                    href="{{ route('sales-payments.edit', $salesInvoice->id) }}">
                                                    <img src="{{ asset('back/assets/dasheets/img/menu.svg') }}"
                                                        class="img-fluid me-1" alt="" />
                                                    Edit Invoice
                                                </a>
                                                ...
                                                <form class=""
                                                    action="{{ route('sales-payments.destroy', $salesInvoice->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
            ...
            $('#deleteRowTrigger').on("click", function(event) { // triggering delete one by one
                if (confirm("Are you sure you won't be able to revert this!")) {
                    if ($('.deleteRow:checked').length > 0) { // at-least one checkbox checked
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{ route('sales.multiplae.delete') }}",
                            data: {
                                ids
                            },
