    ...
    <div class="content">
        ...
        <div class="container-fluid pt-4 px-4 mb-5">
            <div class="border-bottom">
                <h3 class="all-adjustment text-center pb-2 mb-0">Deposit Category</h3>
            </div>
                    ...
                    <div class="row my-3">
                        <div class="col-md-4 col-12">
                            <div class="input-search position-relative">
                                <input type="text" placeholder="Search Deposit Category"
                        ...
                        <tbody>
                            @forelse ($depositCategories as $depositCategory)
                                <tr class="even">
                                    ...
                                    <td class="align-middle">
                                        {{ $depositCategory->description }}
                                    </td>
                                    <td class="align-middle">
                                        <a class=" text-decoration-none btn edit-category-btn" data-bs-toggle="modal"
                                            data-bs-target="#editModalToggle{{ $depositCategory->id }}">
                                            <img src="{{ asset('back/assets/dasheets/img/edit-2.svg') }}"
                                                class="p-0 me-2 ms-0" alt="" />
                                        </a>
                                        <form class="d-inline delete-deposit-category-form" method="post"
                                            action="{{ route('deposit-categories.destroy', $depositCategory->id) }}">
                                            @csrf
                                            @method('DELETE')

                                            <button type="submit" class="btn text-danger btn-outline-light"
                                                onclick="return confirm('Are you sure to delete deposit-category?')">
        ...
        @foreach ($depositCategories as $depositCategory)
            <!-- Edit Modal STart -->
            <div class="modal fade" id="editModalToggle{{ $depositCategory->id }}" aria-hidden="true"
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form action="{{ route('deposit-categories.update', $depositCategory->id) }}" method="POST"
                                enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <div class="row mb-3">
                                    <div class="col-lg-12">
                                        <div class="form-group">
                                            <label for="name">Name</label>
                                            <input type="text" class="form-control" name="name" id="name"
                                                placeholder="" required value="{{ $depositCategory->name }}">
                                        </div>
                                    </div>
                                ...
                                <div class="form-group mb-4">
                                    ...
                                    <textarea class="form-control" id="description" name="description" rows="3">{{ $depositCategory->description }}</textarea>
                                </div>
            ...
            $('#deleteRowTrigger').on("click", function(event) { // triggering delete one by one
                if (confirm("Are you sure you won't be able to revert this!")) {
                    if ($('.deleteRow:checked').length > 0) { // at-least one checkbox checked
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{ route('deposit.category.delete') }}",
                            data: {
                                ids
                            },
