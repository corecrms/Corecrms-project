        ...
        <div class="container-fluid px-4">
                                    ...
                                    <div class="timeline-block d-inline-flex">
                                        <div class="timeline-content">
                                            ...
                                            <small><i class="fas fa-clock me-1"></i> Created on
                                                {{ getDateFormat($purchaseInvoice->created_at) }}</small>
                                        </div>
                                    </div>
                                    <div class="timeline-content float-end">
                                        <a href="{{ route('sales-invoices.edit', $purchaseInvoice) }}"
                                            class="edit-icon btn btn-primary btn-sm mx-1" title="Edit"><i
                                                class="fas fa-pencil-alt me-1"></i> Edit</a>

                                        @if ($purchaseInvoice->status == 'Draft')
                                            <a href={{-- "{{ route('sales-invoices.sendInvoiceByEmail', $purchaseInvoice) }}" --}} class="edit-icon btn btn-success btn-sm mx-1"
            ...
            <div class="row justify-content-between align-items-center mb-3">
                <div class="col-md-12 d-flex align-items-center justify-content-between justify-content-md-end">
                    ...
                    @if (!($purchaseInvoice->status == 'Draft'))
                        <div class="all-button-box mx-2">
                            <a href={{-- "{{ route('order.sendInvoiceByEmail', $purchaseInvoice) }}" --}}
                                class="btn btn-xs btn-white btn-outline-danger btn-icon-only width-auto">Resend Invoice</a>
                        </div>
                    @endif
                    <div class="all-button-box">
                        <a href={{-- "{{ route('order.pdf', $purchaseInvoice) }}" target="_blank" --}}
                                ...
                                <div class="invoice-print">
                                    <div class="row invoice-title mt-2">
                                        ...
                                        <div class="col-xs-12 col-sm-12 col-nd-6 col-lg-6 col-12 text-end">
                                            <h3 class="invoice-number">{{ $purchaseInvoice->invoice_number }}</h3>
                                        </div>
                                        <div class="col-12">
                                            <hr>
                                        </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-4">
                                            ...
                                            <span class="badge  bg-info">{{ $purchaseInvoice->status }}</span>
                                        </div>
                                        <div class="col-md-4 text-md-center">
                                            <strong>Issue Date :</strong><br>
                                            {{-- {{ getDateFormat($purchaseInvoice->issue_date) }} --}}
                                        </div>
                                        <div class="col-md-4 text-md-end">
                                            <strong>Due Date :</strong><br>
                                            {{-- {{ getDateFormat($purchaseInvoice->due_date) }} --}}
                                        </div>
                                    </div>
                                    <div class="row mt-4">
                                        <div class="col-md-12">
                                            ...
                                            <div class="table-responsive mt-2">
                                                ...
                                                <table class="table mb-0 table-striped invoice-table">
                                                    ...
                                                    <tbody>
                                                        @foreach ($purchaseInvoice->purchaseInvoiceDetails as $key => $item)
                                                            <tr>
                                                                <td>{{ ++$key }}</td>
                                                                <td>{{ $item->product->name }}</td>
                                                                <td class="qty text-start">{{ $item->qty }}</td>
                                                                <td class="price">RS{{ $item->price }}</td>
                                                                <td> RS<span class="discount">
                                                                        {{ $item->discount ? $item->discount : 0.0 }}
                                                                    </span>

                                                                </td>
                                                                <td>RS{{ number_format($item->price * $item->qty, 2) }}
                                                                </td>
                                                            </tr>
                                                        @endforeach
                                                        <tr>
                                                        ...
                                                        <tr class="tr2">
                                                            ...
                                                            <td class="text-right">
                                                                RS{{ $purchaseInvoice->paid_amount ? $purchaseInvoice->paid_amount : '0.00' }}
                                                            </td>
                                                        </tr>
                                                        <tr class="tr2">
                                                            <td></td>
                                                            ...
                                                            <td class="text-right">RS
                                                                {{ $purchaseInvoice->creditNotes ? $purchaseInvoice->purchaseInvoiceCreditNotes->sum('amount') : '0.00' }}
                                                            </td>
                                                        </tr>
                                                        <tr class="tr2">
                                                            <td></td>
                                                            ...
                                                            <td class="text-right">
                                                                RS
                                                                @if ($purchaseInvoice->net_amount - $purchaseInvoice->paid_amount > 0)
                                                                    {{ $purchaseInvoice->net_amount - $purchaseInvoice->paid_amount }}
                                                                @else
                                                                    0.00
                                                                @endif
                                                            </td>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped">
                                <tbody>
                                    ...
                                    @foreach ($purchaseInvoice->purchaseInvoicePayments as $payment)
                                        <tr>
                                            ...
                                            <td>
                                                <form method="POST" class="delete-payment-form"
                                                    action="{{ route('sales-invoices.payment.destroy', ['purchaseInvoice' => $purchaseInvoice, 'purchaseInvoicePayment' => $payment]) }}"
                                                    id="delete-form">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-white btn-transparent btn-icon"
                                                        title="Delete"><i class="fas fa-trash text-danger"></i></button>
                                                </form>
                                            </td>
                                        </tr>
                                    @endforeach
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped">
                                <tbody>
                                    ...
                                    @if ($purchaseInvoice->purchaseInvoiceCreditNotes)
                                        @foreach ($purchaseInvoice->purchaseInvoiceCreditNotes as $creditNote)
                                            <tr>
                                                <td>{{ getDateFormat($creditNote->credit_date) }}</td>
                                                <td>RS{{ $creditNote->amount }}</td>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form method="POST" action= "{{ route('sales-invoices.payment.store', $purchaseInvoice) }}">
                        @csrf
                        <div class="row">
                            ...
                            <div class="form-group col-md-6 mb-2">
                                ...
                                <div class="input-group">
                                    ...
                                    <input class="form-control" name="amount" type="text"
                                        value="{{ $purchaseInvoice->net_amount - $purchaseInvoice->paid_amount }}"
                                        id="amount" required>
                                </div>
                            </div>
                            <div class="form-group col-md-6 mb-2">
...
@section('scripts')
    ...
    <script>
        ...
        function renderTotals() {
            ...
            $('.price').each(function() {
                let price = $(this).text().replace('RS', '') || 0;
                price = parseFloat(price).toFixed(2);
                totalPrice += parseFloat(price);
            });
            $('#total_price b').text("RS" + totalPrice.toFixed(2));
            ...
            $('#total_discount b').text("RS" + totalDiscount.toFixed(2));
        }
