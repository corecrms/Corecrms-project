...
@section('title', 'All Purchase Payments')
@section('style')
    ...
    <div class="content">
        <div class="container-fluid pt-4 px-4 mb-5">
            <div class="border-bottom">
                <h3 class="all-adjustment text-center pb-2 mb-0">All Purchase Payments</h3>
            </div>
            @if (session('success'))
                ...
                <div class="card-header border-0 bg-white">
                    ...
                    <div class="row my-3">
                        ...
                        <div class="col-md-8 col-12 text-end">
                            ...
                            <a href="{{ route('purchase-payments.create') }}" class="btn create-btn rounded-3 mt-2">Create
                    ...
                    <table class="table " id="example">
                        <thead class="fw-bold">
                            <tr>
                                ...
                                <th class="text-secondary">Purchase No</th>
                        ...
                        <tbody>
                            @forelse ($purchaseInvoicePayments as $purchaseInvoice)
                                <tr>
                                    ...
                                    {{-- @dd($purchaseInvoice->saleInvoice ) --}}
                                    <td class="align-middle">
                                        {{ $purchaseInvoice->purchaseInvoice->invoice_number }}
                                    </td>
                                    <td class="align-middle">
                                        {{ $purchaseInvoice->purchaseInvoice->purchase->vendor->user->name ?? '' }}</td>
                                    <td class="align-middle">${{ $purchaseInvoice->purchaseInvoice->purchase->grand_total ?? '' }}</td>
                                    <td class="align-middle">${{ $purchaseInvoice->paid_amount }}</td>
                                    <td class="align-middle">{{ $purchaseInvoice->purchasePayment->account->name ?? '...' }}</td>
                                    <td class="align-middle">{{ $purchaseInvoice->purchasePayment->payment_date }}</td>
                                    <td class="align-middle">
                                        {{ $purchaseInvoice->purchasePayment->status === 1 ? 'Yes' : 'No' }}</td>
                                    <td class="align-middle">
                                        <div class="">
                                            ...
                                            <div class="dropdown-menu " aria-labelledby="dropdownMenuLink">
                                                ...
                                                <a class="dropdown-item"
                                                    href="{{ route('purchase-payments.edit', $purchaseInvoice->id) }}">
                                                    <img src="{{ asset('back/assets/dasheets/img/menu.svg') }}"
                                                        class="img-fluid me-1" alt="" />
                                                    Edit Payment
                                                </a>
                                                ...
                                                <form class=""
                                                    action="{{ route('purchase-payments.destroy', $purchaseInvoice->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
    ...
    <script>
            ...
            $('#deleteRowTrigger').on("click", function(event) { // triggering delete one by one
                if (confirm("Are you sure you won't be able to revert this!")) {
                    if ($('.deleteRow:checked').length > 0) { // at-least one checkbox checked
                        ...
                        $.ajax({
                            type: "POST",
                            url: "{{ route('purchase-payment.multiplae.delete') }}",
                            data: {
                                ids
                            },
        ...
        $(document).ready(function() {
            ...
            $(".delete-payment-form").submit(function() {
                var decision = confirm("Are you sure, You want to Delete this purchase payment?");
                if (decision) {
                    return true;
                }
                return false;
